...
class discriminator:
    ...
    def __init__(self):
        ...
        self.W_conv3 = weight_variable([5, 5, 64, 1024])
        self.b_conv3 = bias_variable([1024])
        self.h_conv3 = tf.nn.relu(conv2d(self.h_pool2, self.W_conv3) + self.b_conv3)
        self.h_pool3 = max_pool_2x2(self.h_conv3)

        self.W_fc1 = weight_variable([7 * 7 * 1024, 128])
        self.b_fc1 = bias_variable([128])
        self.h_pool3_flat = tf.reshape(self.h_pool3, [-1, 7 * 7 * 1024])
        self.h_fc1 = tf.nn.relu(tf.matmul(self.h_pool3_flat, W_fc1) + b_fc1)

        self.keep_prob = tf.placeholder(tf.float32)
        self.h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)

        self.W_fc2 = weight_variable([128, 1])
